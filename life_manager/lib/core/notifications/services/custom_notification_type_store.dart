...
class CustomNotificationTypeStore {
  ...
  Future<void> initialize() async {
    ...
    _box = await Hive.openBox<HubCustomNotificationType>(_boxName);
  }

  /// Get all custom types for a specific module.
  Future<List<HubCustomNotificationType>> getAllForModule(String moduleId) async {
    await initialize();
    return _box!.values
  ...
  Future<void> save(HubCustomNotificationType type) async {
    ...
    if (existing != null) {
      ...
      existing
        ...
        ..deliveryConfigJson = Map<String, dynamic>.from(type.deliveryConfigJson)
  ...
  Future<bool> delete(String typeId) async {
    ...
    final entry = _box!.values
        .cast<HubCustomNotificationType?>()
        .firstWhere(
          (t) => t?.id == typeId,
          orElse: () => null,
        );
