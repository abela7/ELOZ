...
final monthlyHabitStatsProvider =
    FutureProvider.family<Map<DateTime, double>, ({int year, int month})>((
      ref,
      params,
    ) async {
...
final dailyHabitStatsProvider = FutureProvider.family<DailyHabitStats, DateTime>((
  ref,
  date,
) async {
  final trace = PerfTrace('HabitsProvider.dailyStats');
  final habits = ref.watch(habitListValueProvider);
  final activeHabits = habits
      .where((h) => !h.isArchived && !h.shouldHideQuitHabit)
      .toList();
  ...
  trace.step(
    'habits_filtered',
    details: {'active': activeHabits.length, 'due': habitsForDate.length},
  );
  ...
  trace.step(
    'statuses_ready',
    details: {'isToday': isToday, 'statuses': dayStatuses.length},
  );
  ...
  switch (habit.completionType) {
    ...
    case 'timer':
      if (completion.actualDurationMinutes != null) {
        ...
        if (target <= 0)
          return completion.actualDurationMinutes! > 0 ? 1.0 : 0.0;
    ...
    case 'quit':
      if (completion.answer != null)
        return completion.answer == true ? 1.0 : 0.0;
