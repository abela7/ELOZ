...
import '../widgets/hub_task_scheduled_section.dart';
import 'hub_orphaned_notifications_page.dart';

/// Task Module Notification Management Page
///
/// Overview, Scheduled, and Settings for Task Manager reminders.
/// Aligns with Finance and Sleep module pages.
class HubTaskModulePage extends StatefulWidget {
  const HubTaskModulePage({super.key});

  @override
  State<HubTaskModulePage> createState() => _HubTaskModulePageState();
}

class _HubTaskModulePageState extends State<HubTaskModulePage>
  ...
  Future<void> _load() async {
    ...
    _settings = await _hub.getModuleSettings(NotificationHubModuleIds.task);
    _scheduledCount = await _hub.getScheduledCountForModule(
      NotificationHubModuleIds.task,
    );
    _orphanedCount = await _countOrphanedNotifications('task');
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              result.success
                  ? 'Synced: ${result.taskRescheduled} task reminders rescheduled'
  ...
  Future<void> _clearAllScheduled() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Clear All Task Notifications?'),
        content: const Text(
          'This will cancel all scheduled task reminders. '
          'You can re-sync them from the Task Manager or here.',
        ),
        actions: [
          TextButton(
    ...
    return Scaffold(
      ...
      appBar: AppBar(
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                color: _taskColor.withOpacity(0.15),
        ...
        bottom: TabBar(
          ...
          labelColor: _taskColor,
          unselectedLabelColor: theme.colorScheme.onSurfaceVariant,
          indicatorColor: _taskColor,
          indicatorWeight: 3,
          tabs: const [
            Tab(text: 'Overview'),
            Tab(text: 'Scheduled'),
          ...
          child: Column(
            children: [
              ListTile(
                ...
                title: const Text(
                  'Sync All Task Reminders',
                  style: TextStyle(fontWeight: FontWeight.w600, fontSize: 15),
                ),
                subtitle: const Text(
                  'Re-schedule reminders for all tasks',
              ...
              ListTile(
                ...
                subtitle: const Text(
                  'Add or edit tasks and their reminders',
                  style: TextStyle(fontSize: 12),
                ),
                trailing: const Icon(Icons.arrow_forward_ios_rounded, size: 16),
                onTap: () {
              ...
              ListTile(
                ...
                subtitle: const Text(
                  'Cancel all pending task notifications',
                  style: TextStyle(fontSize: 12),
                ),
                trailing: const Icon(Icons.arrow_forward_ios_rounded, size: 16),
        ...
        Container(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                child: Text(
                  'Task reminders are set per task when creating or editing. '
                  'Use Task Manager to add reminders to individual tasks.',
                  style: TextStyle(
                    fontSize: 12,
                    color: isDark ? Colors.white54 : Colors.black54,
                  ),
    ...
    return ListView(
      ...
      children: [
        ...
        ListTile(
          ...
          subtitle: const Text(
            'Sounds, quiet hours, diagnostics',
            style: TextStyle(fontSize: 12),
          ),
          trailing: const Icon(Icons.arrow_forward_ios_rounded, size: 16),
          onTap: () {
            ...
            onPressed: () async {
              final confirmed = await showDialog<bool>(
                ...
                builder: (ctx) => AlertDialog(
                  title: const Text('Reset Task Notification Settings?'),
                  content: const Text(
                    'This will reset Task notification settings in the Hub to defaults. '
      ...
      child: InkWell(
        ...
        onTap: () async {
          await Navigator.of(context).push(
            MaterialPageRoute(
              builder: (_) =>
                  const HubOrphanedNotificationsPage(filterType: 'task'),
            ),
          );
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Notifications for deleted tasks still pending',
                    style: TextStyle(
                      fontSize: 12,
