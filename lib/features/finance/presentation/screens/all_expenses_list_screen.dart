...
/// Grouping modes for the expenses list.
enum _GroupBy { none, category, day, week, month, year }

/// All Expenses List Screen – view all expenses in list/grid with filters,
/// grouping, and tap-to-view/edit/delete.
class AllExpensesListScreen extends ConsumerStatefulWidget {
  const AllExpensesListScreen({super.key});

  @override
  ConsumerState<AllExpensesListScreen> createState() =>
      _AllExpensesListScreenState();
}

class _AllExpensesListScreenState extends ConsumerState<AllExpensesListScreen> {
          ...
          Expanded(
            child: transactionsAsync.when(
              data: (all) {
                ...
                final grouped = _groupExpenses(filtered, categories);
          ...
          GestureDetector(
            onTap: () async {
              ...
              await Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (c) => AddTransactionScreen(initialType: 'expense'),
                ),
              );
  ...
  List<Transaction> _filterExpenses(List<Transaction> all) {
    final range = ExpenseRangeUtils.rangeFor(_selectedDate, _rangeView);
    var filtered = ExpenseRangeUtils.filterExpensesForRange(all, range: range);
  ...
  Map<String, List<Transaction>> _groupExpenses(
  ...
  Future<void> _performBulkDelete() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Delete ${_selectedIds.length} expense(s)? Account balances will be updated.',
        ),
        actions: [
          TextButton(
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('${toDelete.length} expense(s) deleted'),
          backgroundColor: Colors.red.shade400,
  ...
  void _onExpenseLongPress(Transaction t) {
    HapticFeedback.mediumImpact();
    setState(() {
  ...
  void _onExpenseTap(
    ...
    if (_selectedIds.isNotEmpty) {
      ...
      _showExpenseDetail(t, defaultCurrency, categories);
    }
  }
  ...
  Widget _buildListTile(
    ...
    final color = cat != null ? Color(cat.colorValue) : const Color(0xFFFF5252);
    ...
    return Dismissible(
      ...
      child: GestureDetector(
        onLongPress: () => _onExpenseLongPress(t),
        onTap: () => _onExpenseTap(t, defaultCurrency, categories),
        child: Container(
          margin: const EdgeInsets.only(bottom: 8),
          ...
          decoration: BoxDecoration(
            color: _selectedIds.contains(t.id)
                ? (isDark
                      ...
                      : const Color(0xFFCDAF56).withOpacity(0.06))
                : (isDark ? Colors.white.withOpacity(0.03) : Colors.white),
            borderRadius: BorderRadius.circular(16),
            border: Border.all(
              color: _selectedIds.contains(t.id)
                  ? const Color(0xFFCDAF56).withOpacity(0.4)
                  : (isDark
                        ? Colors.white.withOpacity(0.04)
  ...
  void _editExpense(Transaction t) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (c) =>
            AddTransactionScreen(transaction: t, initialType: 'expense'),
      ),
