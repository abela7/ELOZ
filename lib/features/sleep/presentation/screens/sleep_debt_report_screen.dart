...
class _SleepDebtReportScreenState extends ConsumerState<SleepDebtReportScreen> {
          ...
          SliverPadding(
            ...
            sliver: SliverList(
              delegate: SliverChildListDelegate([
                ...
                targetAsync.when(
                  data: (settings) =>
                      _body(isDark, range, settings.targetHours),
  ...
  Widget _body(
    ...
    if (_period == _DebtPeriod.allTime) {
      return FutureBuilder<({List<YearlyDebtEntry> yearly, int totalMinutes})>(
        future: _fetchAllTimeData(service, targetHours),
        builder: (context, snap) {
          ...
          final yearly = snap.data!.yearly;
          final total = snap.data!.totalMinutes;
          final h = total ~/ 60;
          final m = total % 60;
          ...
          return Column(
            children: [
              ...
              _section(
                ...
                child: _infoText(
                  isDark,
                  'This is your current sleep debt balance since ${yearly.isNotEmpty ? yearly.first.year : "records began"}. '
                  'Nights below your ${targetHours.toStringAsFixed(1)}h target add debt, while extra sleep repays debt. '
                  'Missing nights still count as full-target debt.',
                ),
              ),
            ],
          );
      ...
      builder: (context, snap) {
        ...
        return FutureBuilder<dynamic>(
          ...
          builder: (context, prevSnap) {
            final prevMinutes = prevSnap.hasData
                ? _totalFromData(prevSnap.data!)
                : null;
            final trend = prevMinutes != null
                ? (prevMinutes - totalMinutes)
                      .sign // + = improving, - = worsening
                : 0;
  ...
  Future<({List<YearlyDebtEntry> yearly, int totalMinutes})> _fetchAllTimeData(
    SleepDebtReportService service,
    double targetHours,
  ) async {
    final yearly = await service.getAllTimeYearlyBreakdown(
      targetHours: targetHours,
    );
    final totalMinutes = await service.getAllTimeDebtMinutes(
      targetHours: targetHours,
    );
    return (yearly: yearly, totalMinutes: totalMinutes);
  }
  ...
  ({DateTime start, DateTime end})? _previousRange() {
    ...
    switch (_period) {
      ...
      case _DebtPeriod.weekly:
        ...
        return (start: prevMon, end: prevMon.add(const Duration(days: 6)));
      case _DebtPeriod.monthly:
        return (
  ...
  int _totalFromData(dynamic data) {
    if (data is List<DailyDebtEntry>) {
      ...
      for (final e in sorted) {
        debt += e.targetMinutes - e.actualMinutes;
        if (debt < 0) debt = 0;
      }
      return debt;
    }
    if (data is List<WeeklyDebtEntry>)
      return data.fold<int>(0, (s, e) => s + e.debtMinutes);
    if (data is List<MonthlyDebtEntry>)
      return data.fold<int>(0, (s, e) => s + e.debtMinutes);
    if (data is List<YearlyDebtEntry>)
      return data.fold<int>(0, (s, e) => s + e.debtMinutes);
    return 0;
  }
  ...
  Widget _periodComparison(bool isDark, int current, int previous) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: (improving ? AppColors.success : AppColors.error).withOpacity(
          isDark ? 0.08 : 0.06,
        ),
        borderRadius: BorderRadius.circular(16),
        border: Border.all(
  ...
  Widget _quickStats(
    ...
    final targetHitPct = totalNights > 0
        ? (targetHit / totalNights * 100).round()
        : 0;
  ...
  Color _debtHeatColor(bool isDark, int debtMinutes, int targetMinutes) {
    if (debtMinutes <= 0)
      return AppColors.success.withOpacity(isDark ? 0.3 : 0.4);
  ...
  Widget _weekdayPattern(bool isDark, List<DailyDebtEntry> daily) {
    ...
    for (final wd in [1, 2, 3, 4, 5, 6, 7]) {
      ...
      avgs[wd] = list.isEmpty
          ? 0
          : list.reduce((a, b) => a + b) / list.length / 60.0;
    }
  ...
  Widget _observations(
    ...
    if (data is List<DailyDebtEntry>) {
      ...
      final shortSleep = data
          .where((e) => e.hadData && e.debtMinutes > 0)
          .length;

      if (totalMinutes > 0) {
        final h = totalMinutes ~/ 60;
        obs.add(
          'You accumulated ${h}h ${totalMinutes % 60}m of sleep debt this period. '
      ...
      if (missing > 0) {
        obs.add(
          '$missing night${missing != 1 ? 's' : ''} had no logged sleep (counted as full target debt). '
      ...
      if (shortSleep > 0 && withDebt > 0) {
        obs.add(
          '$shortSleep night${shortSleep != 1 ? 's' : ''} you slept but were under your ${targetHours.toStringAsFixed(1)}h target. '
      ...
      if (data.length >= 14) {
        final firstHalf = data
            .take(data.length ~/ 2)
            .fold<int>(0, (s, e) => s + e.debtMinutes);
        ...
        if (diff > 60) {
          final dh = diff ~/ 60;
          obs.add(
            'Your debt increased by ${dh}h ${diff % 60}m in the second half of this period. '
            'Review recent habits—stress, schedule changes, or screen time may be affecting sleep.',
          );
        } else if (diff < -60) {
          final dh = (-diff) ~/ 60;
          obs.add(
            'Great progress! You reduced debt by ${dh}h ${(-diff) % 60}m in the second half. Keep it up!',
          );
        }
      }
      ...
      for (final e in data.where((x) => x.debtMinutes > 0)) {
        byWeekday[e.date.weekday] =
            (byWeekday[e.date.weekday] ?? 0) + e.debtMinutes;
      }
      if (byWeekday.isNotEmpty) {
        final worstWd = byWeekday.entries.reduce(
        ...
        obs.add(
          '${dayNames[worstWd.key]} contributes the most to your debt. '
  ...
  Widget _barChart(bool isDark, dynamic data, double targetHours) {
    ...
    if (data is List<DailyDebtEntry>) {
      ...
      items = data
          .map(
            (e) =>
                (label: DateFormat('d').format(e.date), minutes: e.debtMinutes),
          )
          .toList();
    } else if (data is List<WeeklyDebtEntry>) {
      if (data.isEmpty) {
      ...
      items = data
          .map(
            (e) => (
              label: DateFormat('d').format(e.weekStart),
              minutes: e.debtMinutes,
            ),
          )
          .toList();
    } else if (data is List<MonthlyDebtEntry>) {
      ...
      items = data
          .map(
            (e) => (
              label: DateFormat('MMM').format(DateTime(e.year, e.month)),
              minutes: e.debtMinutes,
            ),
          )
          .toList();
      ...
      items = data
          .map((e) => (label: '${e.year}', minutes: e.debtMinutes))
          .toList();
    }
    if (items.isEmpty) {
      return _insufficientDataSection(
    ...
    return _section(
      isDark,
      _period == _DebtPeriod.daily
          ? 'DAILY DEBT'
          : _period == _DebtPeriod.weekly
          ? 'WEEKLY DEBT'
          : _period == _DebtPeriod.monthly
          ? 'MONTHLY DEBT'
          : 'YEARLY DEBT',
      Icons.bar_chart_rounded,
      child: SizedBox(
  ...
  Widget _trendChart(bool isDark, dynamic data) {
    ...
    if (data is List<DailyDebtEntry>)
      minutesList = data.map((e) => e.debtMinutes).toList();
    else if (data is List<WeeklyDebtEntry>)
      minutesList = data.map((e) => e.debtMinutes).toList();
    else if (data is List<MonthlyDebtEntry>)
      minutesList = data.map((e) => e.debtMinutes).toList();
    ...
    final spots = minutesList
        .asMap()
        .entries
        .map((e) => FlSpot(e.key.toDouble(), e.value / 60.0))
        .toList();
  ...
  Widget _detailList(bool isDark, dynamic data, double targetHours) {
    ...
    if (data is List<DailyDebtEntry>) {
      ...
      rows = data.map((e) {
        ...
        final label =
            '${DateFormat('MMM d').format(e.weekStart)} – ${DateFormat('MMM d').format(end)}';
        return _detailRowRich(
          isDark,
          label,
          e.formattedDebt,
          '${e.nightsWithData} logged',
          '7 nights',
          e.nightsWithData > 0,
        );
      }).toList();
      ...
      rows = data.map((e) {
        ...
        return _detailRowRich(
          ...
          label,
          e.formattedDebt,
          '${e.nightsWithData} nights',
          '${e.nightsInMonth} days',
          e.nightsWithData > 0,
        );
      }).toList();
    } else if (data is List<YearlyDebtEntry>) {
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      hadData
                          ? 'Actual: $subtitle · Target: $targetInfo'
                          : 'No data · Target: $targetInfo',
                      style: TextStyle(
                        fontSize: 11,
                        color: isDark ? Colors.white38 : Colors.black38,
                      ),
  ...
  Widget _insufficientDataSection(
    ...
    return _section(isDark, title, icon, child: _infoText(isDark, message));
  }
