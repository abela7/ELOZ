...
/// Page showing all upcoming scheduled notifications across all modules.
/// Opened when user taps the Active Reminders card on the Overview.
/// Supports filtering by module/section and grouping by entity.
class HubUpcomingNotificationsPage extends StatefulWidget {
  const HubUpcomingNotificationsPage({super.key});

  @override
  State<HubUpcomingNotificationsPage> createState() =>
      _HubUpcomingNotificationsPageState();
}

class _HubUpcomingNotificationsPageState
    extends State<HubUpcomingNotificationsPage> {
  int _refreshKey = 0;
  String? _filterModuleId; // null = All
  String? _filterSection; // For Finance only; null = All sections

  Future<List<Map<String, dynamic>>> _loadNotifications() async {
    return NotificationHub().getAllScheduledNotifications();
  }
  ...
  /// Groups notifications: first by entity, then organizes hierarchically
  /// Module → Section → Entity (e.g. Finance → Bills → Netflix).
  List<_HierarchicalItem> _groupNotifications(
    ...
    for (final n in list) {
      ...
      if (entityId.startsWith('bill:')) {
        ...
        groupKey = entityId.isNotEmpty ? entityId : 'single:$hashCode';
        groupLabel =
    ...
    // Build hierarchical list: Module → Section → Entity groups
    ...
    // Collect all module keys that have data (ordered + unordered).
    final allModuleKeys = <String>[
      ...moduleOrder.where(byModule.containsKey),
      ...byModule.keys.where((k) => !moduleOrder.contains(k)),
    ];

    // When only one module exists, skip the module header.
    // When a module has only one section, skip the section header.
    ...
    for (final mid in allModuleKeys) {
      ...
      // Build ordered section keys for known modules, fallback for others.
      ...
      for (final sec in orderedSections) {
        ...
        if (!singleSection) {
          result.add(_HierarchicalItem.sectionHeader(section: sec));
        }
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'Notifications from Finance, Tasks, Habits and other modules will appear here when scheduled.',
            ...
            return Column(
              children: [
                _FilterBar(
                  ...
                  isFinanceSelected:
                      _filterModuleId == FinanceNotificationContract.moduleId,
                  hub: hub,
                  isDark: isDark,
                  theme: theme,
...
/// Hierarchical list item: module header, section header, or entity group.
class _HierarchicalItem {
  ...
  factory _HierarchicalItem.sectionHeader({required String section}) =>
      _HierarchicalItem._(section: section);
...
class _FilterBar extends StatelessWidget {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (isFinanceSelected && sections.length > 1) ...[
              ...
              child: Row(
                children: [
                  ...
                  ...sections.entries.where((e) => e.key != 'other').map((e) {
                    final label = _sectionDisplayLabel(e.key);
                    return Padding(
  ...
  String _sectionDisplayLabel(String section) {
    switch (section) {
      case FinanceNotificationContract.sectionBills:
        return 'Bills';
      case FinanceNotificationContract.sectionDebts:
...
class _GroupCardState extends State<_GroupCard> {
  ...
  String _sectionLabel(String section) {
    switch (section) {
      ...
      case SleepNotificationContract.sectionWinddown:
        return 'Wind-Down';
      default:
        return section.isNotEmpty ? section : '';
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final sectionLabel = _sectionLabel(group.section);
